...
public class MADN_Board extends Board {
    ...
    public void createEndFields() {
        ...
        for (int i = 0; i < 4; i++) {
            switch (i) {
                ...
                case 0:
                    drawEndFields(startX, startY, 0);
                    startX = 6;
                    startY = 2;
                    break;
                case 1:
                    drawEndFields(startX, startY, 1);
                    startX = 10;
                    startY = 6;
                    break;
                case 2:
                    drawEndFields(startX, startY, 2);
                    startX = 6;
                    startY = 10;
                    break;
                case 3:
                    drawEndFields(startX, startY, 3);
                    break;
                default:
                    break;
            }
    ...
    public void drawEndFields(int startX, int startY, int direction) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (fields.get((fields.size() - 1) - (direction * 4)).getFieldType() != MADN_Field.FIELD_TYPE_END) {
                switch (direction) {
                    case 0:
                        fields.get(39).addNext(newEndField);
                        break;
                    case 1:
                        fields.get(9).addNext(newEndField);
                        break;
                    case 2:
                        fields.get(19).addNext(newEndField);
                        break;
                    case 3:
                        fields.get(29).addNext(newEndField);
                        break;
                    default:
                        break;
                }
    ...
    public void drawInitFields(int startX, int startY, int j) {
        ...
        switch (j) {
            case 0:
                newInitField.addNext(fields.get(0));
                break;
            case 1:
                newInitField.addNext(fields.get(10));
                break;
            case 2:
                newInitField.addNext(fields.get(20));
                break;
            case 3:
                newInitField.addNext(fields.get(30));
                break;
            default:
                break;
        }
